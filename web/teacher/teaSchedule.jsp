	...
	$.ajax({
		...
		success : function(data) {
			...
			for (var i = 0, t = 0; i < data.length; i++) {
				...
				if(Thtml!=''){
				$("#TimeTablesTable tr").eq(Classfestival).find("td").eq(Weekday).html(Thtml+'</br>'+data[i].courseName+" "+data[i].classRoomName); // 将表格第m+1行第n+1列的内容设置为s
			}else{
				$("#TimeTablesTable tr").eq(Classfestival).find("td").eq(Weekday).html(data[i].courseName+" "+data[i].classRoomName); // 将表格第m+1行第n+1列的内容设置为s
			}}
